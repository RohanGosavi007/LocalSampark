...
export default function AdminMarketplaceScreen() {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await fetch(`${API_BASE}/admin/marketplace/products`, {
        headers: {
      ...
      console.warn('Failed to fetch marketplace:', e);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>🛒 Marketplace Audit</Text>
      </View>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={<Text style={styles.emptyText}>No data available for Marketplace Audit</Text>}
        />
      )}
    </SafeAreaView>
  );
