...
export default function TownSquareScreen() {
  ...
  const handleVote = async (pollId, optionId) => {
    try {
      const token = await AsyncStorage.getItem('token');
      const res = await fetch(`${API_V1}/townsquare/polls/${pollId}/vote`, {
        method: 'POST',
        headers: { 
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleAdminAction = async (id, action) => {
    try {
      const token = await AsyncStorage.getItem('token');
      const res = await fetch(`${API_V1}/townsquare/news/${id}/${action}`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const data = await res.json();
      Alert.alert('Success', data.message);
      fetchPendingTips(token);
      if (action === 'approve') fetchNews();
    } catch(e) {}
  };

  const createAdminPoll = async () => {
    if (!newPollQuestion || !newPollOption1 || !newPollOption2) {
      Alert.alert('Error', 'Please fill question and at least 2 options');
      return;
    }
    try {
      const token = await AsyncStorage.getItem('token');
      const options = [
        { id: 1, text: newPollOption1, votes: 0 },
        { id: 2, text: newPollOption2, votes: 0 }
      ];
      ...
      const res = await fetch(`${API_V1}/townsquare/polls`, {
        ...
        body: JSON.stringify({ question: newPollQuestion, options, rewardCoins: 5 })
      });
      const data = await res.json();
      Alert.alert('Success', data.message);
      setNewPollQuestion('');
