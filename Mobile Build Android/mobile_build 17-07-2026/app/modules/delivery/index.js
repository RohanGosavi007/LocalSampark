...
export default function MobileDeliveryAgent() {
  ...
  const fetchJobs = async () => {
    try {
      const res = await fetch(`${API_V1}/delivery/jobs?pincode=400001`);
  ...
  const handleAcceptJob = async (jobId) => {
    try {
      const res = await fetch(`${API_V1}/delivery/jobs/${jobId}/accept`, { method: 'POST' });
      const data = await res.json();
      if (data.success) {
  ...
  const handleCompleteJob = async (jobId) => {
    try {
      const res = await fetch(`${API_V1}/delivery/jobs/${jobId}/complete`, { method: 'POST' });
      const data = await res.json();
      if (data.success) {
