...
const API_URL = process.env.EXPO_PUBLIC_API_URL || (isDev ? Constants.expoConfig?.extra?.API_URL_DEV : Constants.expoConfig?.extra?.API_URL_PROD) || 'https://localsampark-api.onrender.com/api/v1';
...
export default function CheckoutScreen() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/shops/${currentShopId}`)
      .then(res => res.json())
      .then(data => {
    ...
    try {
      ...
      if (paymentMethod === 'RAZORPAY') {
        ...
        try {
          ...
          const verifyRes = await fetch(`${API_URL}/checkout/verify`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
