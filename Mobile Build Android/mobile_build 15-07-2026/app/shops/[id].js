...
import api from '../config/api'; // Assuming a common api utility exists, we'll implement or adjust if needed.
...
export default function ShopDetailScreen() {
  ...
  const fetchShopData = async () => {
    try {
      ...
      const bm = shopRes.data.shop?.category_details?.business_model || shopRes.data?.category_details?.business_model;
      
      if (bm === 'appointment' || bm === 'hybrid') {
          const srvRes = await api.get(`/shops/${id}/services`);
          setServices(srvRes.data || []);
          const staffRes = await api.get(`/shops/${id}/staff`);
