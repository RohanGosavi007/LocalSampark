...
export default function AdminSosScreen() {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await fetch(`${API_BASE}/admin/sos/active`, {
        headers: {
      ...
      console.warn('Failed to fetch sos:', e);
    } finally {
      setLoading(false);
    }
  };
    ...
    <SafeAreaView style={styles.container}>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={<Text style={styles.emptyText}>No data available for SOS Active</Text>}
        />
      )}
    </SafeAreaView>
  );
