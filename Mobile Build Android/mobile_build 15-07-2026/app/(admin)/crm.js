...
export default function AdminCrmScreen() {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await fetch(`${API_BASE}/admin/crm/leads`, {
        headers: {
      ...
      console.warn('Failed to fetch crm:', e);
    } finally {
      setLoading(false);
    }
  };
    ...
    <SafeAreaView style={styles.container}>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={<Text style={styles.emptyText}>No data available for CRM Leads</Text>}
        />
      )}
    </SafeAreaView>
  );
