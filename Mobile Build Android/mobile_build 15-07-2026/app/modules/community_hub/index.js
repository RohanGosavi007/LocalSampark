...
export default function MobileCommunityHub() {
  ...
  const fetchLostItems = async () => {
    try {
      const res = await fetch(`${API_V1}/community-hub/lost/active?pincode=400001`);
  ...
  const fetchGarageItems = async () => {
    try {
      const res = await fetch(`${API_V1}/community-hub/garage/items`);
      const data = await res.json();
  ...
  const fetchWeekendMode = async () => {
    try {
      const res = await fetch(`${API_V1}/community-hub/garage/admin/mode`);
      const data = await res.json();
  ...
  const handlePostLost = async () => {
    try {
      const res = await fetch(`${API_V1}/community-hub/lost/post`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({...lostForm, pincode: '400001'})
      });
      const data = await res.json();
      if (data.success) {
  ...
  const handleResolveLost = async (alertId) => {
    ...
    try {
      const res = await fetch(`${API_V1}/community-hub/lost/${alertId}/resolve`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ finderId: 2 }) 
      });
      const data = await res.json();
      if (data.success) {
  ...
  const handlePostGarage = async () => {
    try {
      const res = await fetch(`${API_V1}/community-hub/garage/post`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleBuyGarage = async () => {
    try {
      ...
      const res = await fetch(`${API_V1}/community-hub/garage/${checkoutModal.id}/buy`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
