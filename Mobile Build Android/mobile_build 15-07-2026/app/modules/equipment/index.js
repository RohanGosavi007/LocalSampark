...
export default function MobileCommunityGarage() {
  ...
  const fetchItems = async () => {
    try {
      const res = await fetch(`${API_V1}/equipment/listings`);
  ...
  const fetchMyRentals = async () => {
    try {
      const res = await fetch(`${API_V1}/equipment/rentals/me`);
      const data = await res.json();
  ...
  const handleRentEquipment = async () => {
    try {
      ...
      const res = await fetch(`${API_V1}/equipment/rent/${bookingModal.id}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload)
      });
  ...
  const handleConfirmReturn = async (rentalId) => {
    try {
      const res = await fetch(`${API_V1}/equipment/rentals/${rentalId}/return`, {
        method: 'POST'
      });
      const data = await res.json();
      if (data.success) {
