...
async function updateRiderStatus(req, res, next) {
  try {
    ...
    await query('UPDATE delivery_riders SET status = $1 WHERE id = $2', [status, riderId]);
...
async function registerRider(req, res, next) {
  try {
    ...
    await query(
      'INSERT INTO delivery_riders (id, name, phone, vehicle_type, vehicle_number, status) VALUES ($1, $2, $3, $4, $5, $6)',
