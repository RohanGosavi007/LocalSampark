...
const createTopic = async (req, res, next) => {
    try {
        ...
        const id = uuidv4();
        await query(
            `INSERT INTO society_forum_topics 
...
const replyToTopic = async (req, res, next) => {
    try {
        ...
        const topic = await queryOne('SELECT is_locked FROM society_forum_topics WHERE id = ?', [topicId]);
        if (topic?.is_locked) {
        ...
        const id = uuidv4();
        await query(
            `INSERT INTO society_forum_replies 
...
const pinTopic = async (req, res, next) => {
    try {
        ...
        await query('UPDATE society_forum_topics SET is_pinned = ? WHERE id = ?', [isPinned ? 1 : 0, topicId]);
...
const lockTopic = async (req, res, next) => {
    try {
        ...
        await query('UPDATE society_forum_topics SET is_locked = ? WHERE id = ?', [isLocked ? 1 : 0, topicId]);
