...
cron.schedule('*/5 * * * *', async () => {
    ...
    try {
        const queryStr = `
            ...
            AND datetime(checked_in_at, '+' || max_stay_minutes || ' minutes') < CURRENT_TIMESTAMP
        `;
        ...
        for (const visitor of overstayers) {
            ...
            // Mark as alert sent
            await query('UPDATE society_visitors SET overstay_alert_sent = 1 WHERE id = ?', [visitor.id]);
            ...
            const member = await query('SELECT user_id FROM society_members WHERE society_id = ? AND flat_number = ?', [visitor.society_id, visitor.flat_number]);
