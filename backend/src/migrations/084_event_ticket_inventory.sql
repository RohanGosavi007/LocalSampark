-- Migration 084: ticket inventory for events
--
...
ALTER TABLE events ADD COLUMN IF NOT EXISTS total_tickets INTEGER DEFAULT 0;
ALTER TABLE events ADD COLUMN IF NOT EXISTS available_tickets INTEGER DEFAULT 0;
ALTER TABLE events ADD COLUMN IF NOT EXISTS allow_coin_discount BOOLEAN DEFAULT FALSE;

UPDATE events
   SET total_tickets = COALESCE(max_attendees, 0),
       available_tickets = GREATEST(COALESCE(max_attendees, 0) - COALESCE(current_attendees, 0), 0)
