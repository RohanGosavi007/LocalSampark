-- SQLite variant of 090_multitenant_region_indexes.sql. See that file for why.
--
-- Differences: no explicit transaction (the runner executes statement by
-- statement), and no DESC in index definitions — SQLite can walk an index
-- backwards, so a plain ascending index serves ORDER BY ... DESC equally well.
--
...
CREATE INDEX IF NOT EXISTS idx_users_region_active ON users(region_id, is_active, created_at);
...
CREATE INDEX IF NOT EXISTS idx_revenue_tx_region_created ON revenue_transactions(region_id, created_at);
