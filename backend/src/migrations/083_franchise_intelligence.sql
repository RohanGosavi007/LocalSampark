...
ALTER TABLE franchise_lead_crm ADD COLUMN IF NOT EXISTS region_id VARCHAR(255) REFERENCES regions(id);
...
CREATE TABLE IF NOT EXISTS franchise_outreach_log (
    ...
    shop_id    VARCHAR(255) REFERENCES local_shops(id) ON DELETE SET NULL,
    method     VARCHAR(100),
    notes      TEXT,
    logged_by  VARCHAR(255) REFERENCES users(id) ON DELETE SET NULL,
