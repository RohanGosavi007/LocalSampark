-- Migration 086 (SQLite): the last columns the application queries but never had
--
...
ALTER TABLE marketplace_chat_messages ADD COLUMN listing_id TEXT REFERENCES marketplace_listings(id);
...
ALTER TABLE shop_staff ADD COLUMN availability TEXT;

-- events.routes.js lists local events by lifecycle state ('upcoming'), which
-- is_active cannot express.
ALTER TABLE local_events ADD COLUMN status TEXT DEFAULT 'upcoming';
...
ALTER TABLE job_skills ADD COLUMN resume_id TEXT;
