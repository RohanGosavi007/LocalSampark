...
// POST /api/v1/campaigns/:shopId - Create campaign
router.post('/:shopId', authenticate, async (req, res) => {
  try {
    ...
    res.status(201).json({ message: 'Campaign scheduled', campaign: result.rows[0] });
  } catch (error) {
...
router.get('/:shopId', authenticate, async (req, res) => {
  try {
    const { shopId } = req.params;
    const result = await pool.query(`SELECT * FROM shop_campaigns WHERE shop_id = $1 ORDER BY start_datetime DESC`, [shopId]);
...
router.put('/:shopId/:campaignId/activate', authenticate, async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Server error activating campaign' });
  }
});
