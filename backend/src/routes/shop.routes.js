...
const { autoCreateShopDelivery } = require('../controllers/delivery.controller');
...
router.get('/nearby', async (req, res, next) => {
  try {
    ...
    let shopsQuery = "SELECT * FROM local_shops WHERE is_active = 1 AND approval_status = 'approved'";
    const params = [];
    let paramIdx = 1;
    ...
    // Filter and sort by distance ONLY if strict region filtering is not applied
    let shopsWithDistance = shops.map(shop => {
        const dist = getDistanceFromLatLonInKm(parseFloat(lat), parseFloat(lng), shop.latitude, shop.longitude);
        return { ...shop, distance_km: parseFloat(dist.toFixed(2)) };
    });

    if (!region_id) {
        shopsWithDistance = shopsWithDistance.filter(shop => shop.distance_km <= radius);
    }
    
    shopsWithDistance.sort((a, b) => a.distance_km - b.distance_km);

    // Sort premium shops to top
    shopsWithDistance.sort((a, b) => b.is_premium - a.is_premium);
...
const { upload, recordUpload } = require('../services/upload.service');
