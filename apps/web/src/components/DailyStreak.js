...
export default function DailyStreak() {
  ...
  return (
    <div className="bg-background border border-border rounded-2xl p-6 shadow-xl relative overflow-hidden mb-6">
      ...
      <div className="flex justify-between items-start mb-6">
        <div>
          ...
          <p className="text-text-muted text-sm mt-1">Check in for 7 days to unlock a Mystery Box.</p>
        </div>
      </div>

      {/* Week Timeline */}
      <div className="flex justify-between items-center mb-6 relative">
        <div className="absolute top-1/2 left-0 right-0 h-1 bg-card-bg -z-10 -translate-y-1/2 rounded-full"></div>
          ...
          return (
            <div key={i} className="flex flex-col items-center gap-2">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm transition-all shadow-lg ${
                ...
                isToday ? 'bg-card-bg border-2 border-orange-500 text-orange-500 shadow-[0_0_15px_rgba(249,115,22,0.4)]' :
                'bg-card-bg text-text-muted border border-border'
              }`}>
