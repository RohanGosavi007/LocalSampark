      ...
      <div className="max-w-4xl mx-auto p-6 pt-24">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-4">
          <div>
            ...
            <p className="text-text-muted mt-2 font-medium">Manage your recurring morning deliveries. Pause anytime.</p>
          </div>
        ...
        <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-2xl p-5 mb-8 flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-text-muted text-sm mb-3">You can easily pause all your morning deliveries in one click to prevent wastage.</p>
            <button className="bg-card-bg border border-yellow-500/50 hover:border-yellow-500 text-yellow-500 text-xs font-bold px-4 py-2 rounded-lg transition-colors">
        ...
        <div className="space-y-4">
          {subs.map(sub => (
            <div key={sub.id} className={`bg-card-bg border-2 rounded-3xl p-6 transition-all ${sub.status === 'active' ? 'border-border hover:border-blue-500' : 'border-border opacity-75'}`}>
              
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                
                <div className="flex items-center gap-4">
                  <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 ${sub.status === 'active' ? 'bg-blue-900/30 text-blue-500' : 'bg-background-alt text-text-muted border border-border'}`}>
                    <Package size={24} />
                  </div>
                  <div>
                    <div className="flex items-center gap-3 mb-1">
                      <h3 className="text-xl font-bold text-text">{sub.item}</h3>
                      {sub.status === 'active' ? (
                ...
                <div className="flex items-center gap-3 w-full md:w-auto">
                  <button className="flex-1 md:flex-none bg-card-bg hover:bg-background-alt border border-border text-text font-bold py-3 px-6 rounded-xl transition-colors text-sm">
                    Modify
                  </button>
                  <button 
