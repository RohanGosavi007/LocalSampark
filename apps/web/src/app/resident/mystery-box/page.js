    ...
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center p-6 text-text text-center">
        ...
        <div className="bg-card-bg border border-border rounded-2xl p-6 w-full max-w-sm mb-8 text-left">
          ...
          <p className="text-text-muted text-sm mb-4">Between 6 PM - 9 PM</p>

          <div className="border-t border-border pt-4 flex justify-between items-center text-sm">
            <span className="text-text-muted">Auto-Renews Weekly</span>
            <span className="font-bold text-text">₹{boxes[selectedBox].price}/wk</span>
          </div>
        </div>

        <button onClick={() => router.push('/resident')} className="bg-card-bg text-text font-bold py-3 px-8 rounded-full hover:bg-background-alt transition-colors">
          Back to Dashboard
        </button>
      </div>
    );
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto mb-12">
          ...
          {Object.values(boxes).map((b) => (
            <div 
              ...
              className={`relative cursor-pointer transition-all duration-300 bg-card-bg rounded-3xl border-2 overflow-hidden ${
                selectedBox === b.id
            ...
            >
              ...
              <div className="p-8">
                ...
                <p className="text-text-muted text-sm h-16">{b.description}</p>

                <div className="my-6 pt-6 border-t border-border">
                  <p className="text-xs text-text-muted font-bold uppercase tracking-wider mb-3 flex items-center gap-1"><Info size={14}/> What to expect</p>
                ...
                <div className="mt-8 flex items-end justify-between bg-background p-4 rounded-2xl border border-border">
                  <div>
                    <p className="text-text-muted text-xs font-bold uppercase tracking-wider mb-1">Value: ₹{b.value}</p>
                    <p className="text-3xl font-black">₹{b.price}<span className="text-base text-text-muted font-medium">/wk</span></p>
                  </div>
                </div>
              </div>
            </div>
