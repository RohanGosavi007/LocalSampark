        ...
        <div className="lg:col-span-3 space-y-4">
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              {/* OPEN */}
              <div className="bg-slate-900 border rounded-3xl p-4 shadow-xl" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border-color)' }}>
                ...
                <div className="space-y-3">
                  {filteredTickets.filter(t => t.status === 'open').map(t => (
                    <div key={t.id} className="p-3 border rounded-xl bg-slate-950" style={{ borderColor: 'var(--border-color)', backgroundColor: 'var(--bg-base)' }}>
                      <span className="text-[10px] uppercase font-bold text-slate-500 mb-1 block">{t.domain}</span>
                      <h4 className="text-sm font-bold truncate text-white">{t.subject}</h4>
              ...
              {/* RESOLVED */}
              <div className="bg-slate-900 border rounded-3xl p-4 shadow-xl opacity-75" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border-color)' }}>
                ...
                <div className="space-y-3">
                  {filteredTickets.filter(t => t.status === 'resolved').map(t => (
                    <div key={t.id} className="p-3 border rounded-xl bg-slate-950" style={{ borderColor: 'var(--border-color)', backgroundColor: 'var(--bg-base)' }}>
                      <span className="text-[10px] uppercase font-bold text-slate-500 mb-1 block">{t.domain}</span>
                      <h4 className="text-sm font-bold truncate line-through text-slate-500">{t.subject}</h4>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="bg-slate-900 border rounded-3xl p-6 shadow-xl" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border-color)' }}>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between p-3 border rounded-xl" style={{ borderColor: 'var(--border-color)' }}>
                <div>
                  <h4 className="text-sm font-bold text-white">Enable Bot</h4>
                  <p className="text-[10px] text-slate-400">Instantly reply to new tickets.</p>
                </div>
                <button 
              ...
              <div>
                <label className="text-xs font-bold uppercase tracking-wider mb-1 block text-slate-500">Bot Message</label>
                <textarea 
                  rows={4}
