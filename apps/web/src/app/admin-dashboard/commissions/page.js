...
export default function CommissionHub() {
  ...
  if (loading) return <div className="p-8 text-slate-900 dark:text-white">Loading Commission Hub...</div>;

  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Commission Hub</h1>

      <div className="grid grid-cols-4 gap-4">
        <div className="bg-slate-100 dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700">
            <div className="text-sm text-slate-500 dark:text-slate-400">Total Comm. Earned</div>
            <div className="text-2xl font-bold text-emerald-400">₹{data.total_earned}</div>
        </div>
        <div className="bg-slate-100 dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700">
            <div className="text-sm text-slate-500 dark:text-slate-400">Total Conv. Fees</div>
            <div className="text-2xl font-bold text-emerald-400">₹{data.total_convenience}</div>
        </div>
        <div className="bg-slate-100 dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700">
            <div className="text-sm text-slate-500 dark:text-slate-400">Pending Settlements</div>
            <div className="text-2xl font-bold text-yellow-400">₹{data.pending_settlements}</div>
        </div>
        <div className="bg-slate-100 dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700">
            <div className="text-sm text-slate-500 dark:text-slate-400">Settlements Done</div>
            <div className="text-2xl font-bold text-slate-700 dark:text-slate-200">₹{data.settlements_done}</div>
        </div>
      </div>

      <div className="bg-slate-100 dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-lg font-bold text-slate-900 dark:text-white">Per-Shop Commission & Override</h2>
        ...
        <table className="w-full text-left text-sm text-slate-600 dark:text-slate-300">
            ...
            <tbody>
                {data.shops.map(shop => (
                    <tr key={shop.id} className="border-b border-slate-200 dark:border-slate-700">
                        <td className="p-3 font-semibold text-slate-900 dark:text-white">{shop.name}</td>
