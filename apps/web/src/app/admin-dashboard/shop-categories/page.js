  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-bold text-white mb-2">Shop Categories</h1>
          <p className="text-slate-400">Manage the universal catalog taxonomy for all registered shops.</p>
        </div>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl mb-8 flex flex-col md:flex-row gap-4 items-center">
        <input 
          ...
          className="bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-blue-500 flex-1 w-full"
        />
        <input 
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {loading ? (
            <div className="col-span-3 text-center py-8 text-slate-500">Loading categories...</div>
          ) : categories.map(cat => (
            <div key={cat.id} className="p-4 bg-slate-950 border border-slate-800 rounded-2xl flex items-center justify-between group hover:border-blue-500/50 transition cursor-pointer">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-slate-500 text-xs">/{cat.slug || cat.name.toLowerCase().replace(/\s+/g, '-')}</p>
                </div>
              </div>
              <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition">
                <button className="p-2 text-slate-400 hover:text-white transition"><Edit2 className="w-4 h-4"/></button>
