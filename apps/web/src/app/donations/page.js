    ...
    <>
      ...
      <main className="min-h-screen bg-slate-950 pt-20 pb-16">
        ...
        <div className="max-w-6xl mx-auto px-4 py-12">
          <h2 className="text-2xl font-bold text-white mb-6">Active Requests Near You</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {loading ? (
              ...
              requests.map((req, i) => (
                <motion.div key={req.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} className="bg-slate-900 border border-slate-800 rounded-3xl p-6 hover:border-rose-500/50 transition-all flex flex-col relative overflow-hidden">
                  ...
                  <div className="flex items-center gap-4 mb-4 mt-2">
                    ...
                    <div>
                      <h3 className="text-lg font-bold text-white leading-tight">{req.title}</h3>
                      <span className="text-sm text-slate-400">{req.org}</span>
                    </div>
                  </div>
                  ...
                  <div className="mb-6 space-y-2">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="text-white font-bold">{Math.round((req.collected/parseInt(req.target))*100) || 60}%</span>
                    </div>
                    ...
                    <div className="text-xs text-slate-500 text-right">{req.collected} of {req.target}</div>
                  </div>
