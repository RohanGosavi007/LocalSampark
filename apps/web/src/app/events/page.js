      ...
      <main className="min-h-screen bg-background pt-20 pb-16">
        ...
        <section className="relative overflow-hidden py-16 px-4">
          ...
          <div className="max-w-6xl mx-auto relative z-10 text-center">
            ...
            <div className="flex items-center gap-3 max-w-xl mx-auto bg-background-alt/80 backdrop-blur-md rounded-2xl border border-border px-4 py-3">
              ...
              <input type="text" placeholder="Search events..." className="bg-transparent text-text flex-1 outline-none placeholder:text-text-muted" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
            </div>
          </div>
        </section>
        ...
        <div className="max-w-6xl mx-auto px-4 mb-8 overflow-x-auto pb-4 hide-scrollbar">
          <div className="flex gap-3 justify-center min-w-max">
            {CATEGORIES.map(c => (
              <button key={c.id} onClick={() => setCategory(c.id)} className={`px-5 py-2.5 rounded-full text-sm font-semibold transition-all ${category === c.id ? 'bg-teal-600 text-white shadow-lg shadow-teal-600/30' : 'bg-background-alt text-text-muted hover:bg-border border border-border'}`}>
                {c.label}
              </button>
            ))}
          </div>
        ...
        <div className="max-w-6xl mx-auto px-4">
          {loading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {[1,2,3].map(i => (
                <div key={i} className="bg-background-alt/60 rounded-3xl h-80 animate-pulse" />
              ))}
            </div>
          ) : filtered.length === 0 ? (
            <div className="text-center py-20">
              <Calendar className="w-16 h-16 text-text-muted mx-auto mb-4" />
              <h3 className="text-xl text-text-muted font-semibold">No upcoming events</h3>
              <p className="text-text-muted mt-2">Check back later or explore other categories.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filtered.map((event, i) => (
                <motion.div key={event.id || i} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="bg-card-bg border border-border rounded-3xl overflow-hidden group hover:border-teal-500/50 hover:shadow-xl hover:shadow-teal-500/10 transition-all cursor-pointer flex flex-col" onClick={() => setSelectedEvent(event)}>
                  <div className="h-48 bg-background-alt relative">
                    ...
                    <div className="absolute top-4 right-4 bg-card-bg/90 backdrop-blur-sm px-3 py-2 rounded-xl border border-border text-center">
                      <div className="text-xs text-teal-400 font-bold uppercase">{new Date(event.event_date).toLocaleString('default', { month: 'short' })}</div>
                      <div className="text-xl text-text font-black leading-none">{new Date(event.event_date).getDate()}</div>
                    </div>
                  </div>
                  <div className="p-6 flex-1 flex flex-col">
                    ...
                    <h3 className="text-xl font-bold text-text mb-3 line-clamp-2">{event.title}</h3>
                    <div className="space-y-2 mb-6 mt-auto">
                      <p className="text-text-muted text-sm flex items-center gap-2"><Clock className="w-4 h-4 text-text-muted" /> {event.start_time || 'TBA'}</p>
                      <p className="text-text-muted text-sm flex items-start gap-2"><MapPin className="w-4 h-4 text-text-muted shrink-0 mt-0.5" /> <span className="line-clamp-1">{event.venue}</span></p>
                    </div>
                  </div>
                </motion.div>
              ))}
        ...
        <AnimatePresence>
          {selectedEvent && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto" onClick={() => setSelectedEvent(null)}>
              <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} onClick={(e) => e.stopPropagation()} className="bg-card-bg border border-border rounded-3xl w-full max-w-2xl overflow-hidden shadow-2xl relative my-8">
                ...
                <div className="p-6 sm:p-8 -mt-20 relative z-10">
                  <div className="bg-background-alt/80 backdrop-blur-md rounded-2xl p-6 border border-border flex justify-between items-center mb-8 shadow-xl">
                    <div className="flex items-center gap-4">
                      <div className="text-center px-4 py-2 bg-background-alt rounded-xl border border-border">
                        ...
                        <div className="text-text font-black text-2xl leading-none">{new Date(selectedEvent.event_date).getDate()}</div>
                      </div>
                      <div>
                        <h2 className="text-2xl font-bold text-text line-clamp-1">{selectedEvent.title}</h2>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    <div className="md:col-span-2 space-y-6">
                      <div>
                        <h3 className="text-lg font-bold text-text mb-2">About Event</h3>
                        <p className="text-text-muted text-sm leading-relaxed whitespace-pre-line">{selectedEvent.description || 'No description provided.'}</p>
                      </div>

                      <div className="bg-background-alt/50 rounded-2xl p-4 border border-border/50 space-y-3">
                        <div className="flex items-start gap-3 text-sm">
                          <MapPin className="w-5 h-5 text-teal-400 shrink-0" />
                          <div>
                            <span className="text-text font-medium block">Venue</span>
                            <span className="text-text-muted">{selectedEvent.venue}</span>
                          </div>
                        </div>
                        <div className="flex items-start gap-3 text-sm">
                          <Clock className="w-5 h-5 text-teal-400 shrink-0" />
                          <div>
                            <span className="text-text font-medium block">Time</span>
                            <span className="text-text-muted">{selectedEvent.start_time} - {selectedEvent.end_time || 'Late'}</span>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="md:col-span-1">
                      <div className="bg-background-alt border border-border rounded-2xl p-5 sticky top-4">
                        <div className="mb-4">
                          <span className="text-text-muted text-sm block">Ticket Price</span>
                          <span className="text-3xl font-black text-text">{selectedEvent.is_paid ? `₹${selectedEvent.ticket_price}` : 'FREE'}</span>
                        </div>
                        ...
                        {booked ? (
                          ...
                          <>
                            <div className="flex items-center justify-between bg-background-alt rounded-xl p-2 mb-4 border border-border">
                              <button onClick={() => setTicketCount(Math.max(1, ticketCount - 1))} className="w-8 h-8 rounded-lg flex items-center justify-center text-text-muted hover:bg-border hover:text-text transition">-</button>
                              <span className="text-text font-bold">{ticketCount}</span>
                              <button onClick={() => setTicketCount(Math.min(10, ticketCount + 1))} className="w-8 h-8 rounded-lg flex items-center justify-center text-text-muted hover:bg-border hover:text-text transition">+</button>
                            </div>

                            <div className="flex justify-between text-sm text-text-muted mb-6 px-1">
                              <span>Total ({ticketCount} ticket{ticketCount > 1 ? 's' : ''})</span>
                              <span className="text-text font-bold">{selectedEvent.is_paid ? `₹${selectedEvent.ticket_price * ticketCount}` : 'FREE'}</span>
                            </div>
