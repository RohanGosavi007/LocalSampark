        ...
        <div className="max-w-6xl mx-auto px-4 py-12">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {FALLBACK_OPPORTUNITIES.map((opp, i) => (
              <motion.div key={opp.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} className="bg-slate-900 border border-slate-800 rounded-3xl p-6 hover:border-emerald-500/50 transition-all flex flex-col">
                <div className="flex justify-between items-start mb-4">
                  <span className="px-3 py-1 bg-emerald-500/10 text-emerald-400 text-xs font-bold uppercase rounded-full">
                    {opp.type}
                  </span>
                  <span className="text-xs text-slate-500 font-bold">{opp.spots} spots left</span>
                </div>
                
                <h3 className="text-xl font-bold text-white mb-1 leading-tight">{opp.title}</h3>
                <span className="text-sm text-slate-400 mb-6 flex items-center gap-1"><ShieldCheck className="w-3.5 h-3.5 text-blue-400" /> {opp.org}</span>
                
                <div className="space-y-3 mb-6 flex-1 bg-slate-800/50 p-4 rounded-2xl border border-slate-700/50">
              ...
              <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} onClick={e => e.stopPropagation()} className="bg-slate-900 border border-slate-700 rounded-3xl w-full max-w-md p-8">
                {applied ? (
                  <div className="text-center py-8">
                    ...
                    <p className="text-slate-400">The organizer will contact you shortly with next steps.</p>
                  </div>
                ) : (
                  <>
                    <h2 className="text-2xl font-bold text-white mb-2">Join Initiative</h2>
                    <p className="text-slate-400 mb-6">{selectedOpp.title}</p>
                    
                    <textarea placeholder="Why do you want to volunteer? (Optional)" className="w-full bg-slate-800 text-white rounded-xl p-4 border border-slate-700 focus:border-emerald-500 outline-none resize-none h-24 mb-6" />
