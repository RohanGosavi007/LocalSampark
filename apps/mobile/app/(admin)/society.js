...
export default function AdminSocietyScreen() {
  ...
  const fetchData = async () => {
    try {
      ...
      console.warn('Failed to fetch society:', e);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>🏘️ Society Audit</Text>
      </View>
      ...
      {loading ? (
        ...
        <FlashList estimatedItemSize={100}
          ...
          ListEmptyComponent={<Text style={styles.emptyText}>No data available for Society Audit</Text>}
        />
      )}
    </SafeAreaView>
  );
