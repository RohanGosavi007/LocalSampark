...
export default function AdminWalletScreen() {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await fetch(`${API_BASE}/admin/wallet/transactions/all`, {
        headers: {
    ...
    <SafeAreaView style={styles.container}>
      ...
      {loading ? (
        ...
        <FlashList estimatedItemSize={100}
          ...
          ListEmptyComponent={<Text style={styles.emptyText}>No data available for Wallet Transactions</Text>}
        />
      )}
    </SafeAreaView>
  );
