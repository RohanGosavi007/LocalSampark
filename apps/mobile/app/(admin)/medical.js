...
export default function AdminMedicalScreen() {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await fetch(`${API_BASE}/admin/medical/records`, {
        headers: {
      ...
      console.warn('Failed to fetch medical:', e);
    } finally {
      setLoading(false);
    }
  };
    ...
    <SafeAreaView style={styles.container}>
      ...
      {loading ? (
        ...
        <FlashList estimatedItemSize={100}
          ...
          ListEmptyComponent={<Text style={styles.emptyText}>No data available for Medical Records</Text>}
        />
      )}
    </SafeAreaView>
  );
