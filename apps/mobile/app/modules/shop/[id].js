...
export default function ShopDetailScreen() {
  ...
  const { items: cart, addItem: addToCart, removeItem: removeFromCart, updateQuantity, getCartTotal } = useCartStore();
                ...
                <View style={styles.productAction}>
                  {(item.track_inventory === 1 && item.inventory_count <= 0) ? (
                    ...
                    <View style={styles.qtyControl}>
                      ...
                      >
                        <Text style={[styles.qtyBtnText, item.track_inventory === 1 && qty >= item.inventory_count && { opacity: 0.5 }]}>+</Text>
                      </TouchableOpacity>
                    </View>
                  )}
                </View>
