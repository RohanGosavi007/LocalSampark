  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1.25rem' }}>
        {[
          { label: 'Total Escrow Balances', val: '₹14,820', sub: 'In-app wallets float', color: 'var(--accent-text)', icon: '🔒' },
          { label: 'Convenience Fees Generated', val: '₹2,450', sub: 'From utility bill checkouts', color: 'var(--success)', icon: '🎟️' },
          { label: 'Pending Payouts', val: '₹8,350', sub: 'To local shop merchants', color: 'var(--warning)', icon: '📤' },
        ].map(stat => (
          <div key={stat.label} style={{ ...cardStyle, borderLeft: `4px solid ${stat.color}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <p style={{ color: 'var(--ink-subtle)', fontSize: '0.75rem', margin: 0, fontWeight: 'bold' }}>{stat.label}</p>
              <h3 style={{ fontSize: '1.5rem', fontWeight: 800, margin: '0.2rem 0' }}>{stat.val}</h3>
              <p style={{ color: 'var(--ink-muted)', fontSize: '0.72rem', margin: 0 }}>{stat.sub}</p>
            </div>
      ...
      <div style={cardStyle}>
        ...
        <p style={{ color: 'var(--ink-subtle)', fontSize: '0.85rem', marginBottom: '1.75rem' }}>Adjust the revenue split percentages. Total must equal 100%.</p>
        {[
          { label: 'Platform Share (LocalSampark)', val: platformShare, setter: setPlatformShare, color: 'var(--accent-text)' },
          { label: 'Franchise Partner Share', val: franchiseShare, setter: setFranchiseShare, color: 'var(--success)' },
          { label: 'Delivery Agent Share', val: agentShare, setter: setAgentShare, color: 'var(--warning)' },
          { label: 'Misc / Reserve Fund', val: miscShare, setter: setMiscShare, color: 'var(--accent-text)' },
        ].map(item => (
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '0.5rem' }}>
          <span style={{ color: totalShare === 100 ? 'var(--success)' : 'var(--danger)', fontWeight: 700 }}>
              ...
              {(revenueChart?.data || []).map(r => {
                ...
                return (
                  <tr key={r.name} style={{ borderBottom: '1px solid var(--line)' }}>
                    ...
                    <td style={{ padding: '0.85rem 1rem', color: 'var(--success)', fontWeight: 700 }}>₹{gross.toLocaleString()}</td>
                    <td style={{ padding: '0.85rem 1rem', color: 'var(--accent-text)' }}>₹{p.toLocaleString()}</td>
                    <td style={{ padding: '0.85rem 1rem', color: 'var(--success)' }}>₹{f.toLocaleString()}</td>
                    <td style={{ padding: '0.85rem 1rem', color: 'var(--warning)' }}>₹{a.toLocaleString()}</td>
