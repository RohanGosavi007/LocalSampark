...
export default function CharityTab({ API_BASE, authHeaders }) {
  ...
  useEffect(() => {
    setData([
      { id: 'c1', module: 'NGO Registrations', count: 14, status: 'Audit Pending' },
      { id: 'c2', module: 'Local Donation Drives', count: 8, status: 'Active' },
      { id: 'c3', module: 'Disaster Volunteers', count: 120, status: 'Active' },
      { id: 'c4', module: 'Health Camp Organizers', count: 3, status: 'Active' },
    ]);
    setLoading(false);
  }, []);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      <div style={cardStyle}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
          <div>
            <h3 style={{ fontSize: '1.1rem', margin: 0, color: 'var(--ink)' }}>🤝 Charity & NGO Management</h3>
            <p style={{ color: 'var(--ink-subtle)', fontSize: '0.85rem', margin: '0.2rem 0 0' }}>Audit donations, volunteers, and local NGO registrations.</p>
          </div>
          <button style={btnPrimary}>Verify NGO List</button>
        </div>
        
        {loading ? <p style={{ color: 'var(--ink-muted)' }}>Loading charity metrics...</p> : (
