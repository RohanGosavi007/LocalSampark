...
export default function WalletTab({ API_BASE, authHeaders }) {
  ...
  const fetchData = async () => {
    ...
    setError(null);
    try {
      const data = await fetchJson(`${API_BASE}/admin/wallet/transactions/all?limit=50`, { headers: authHeaders() });
      ...
      <div style={cardStyle}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
          <div>
            ...
            <p style={{ color: 'var(--ink-subtle)', fontSize: '0.85rem', margin: 0 }}>Monitor platform wallet balances, process payouts to shops and agents, and resolve disputes.</p>
          </div>
          <button onClick={fetchData} style={btnPrimary}>{loading ? 'Loading...' : 'Refresh'}</button>
        </div>

        {/* Append-Only Ledger Alert */}
        <div style={{ background: 'var(--warning-quiet)', border: '1px solid #7c2d12', padding: '1rem', borderRadius: '0.5rem', marginBottom: '1.5rem', color: 'var(--warning)', fontSize: '0.85rem', display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
            ...
            <tbody>
              {dataList.length === 0 ? (
                ...
                <tr key={item.id || i} style={{ borderBottom: '1px solid var(--line)' }}>
                  ...
                  <td style={{ padding: '0.85rem 1rem', color: 'var(--ink-muted)' }}>{item.user_name || '—'}</td>
                  <td style={{ padding: '0.85rem 1rem', color: item.type === 'credit' ? 'var(--success)' : 'var(--danger)', fontWeight: 700 }}>
                    {item.type === 'credit' ? '+' : '-'}₹{item.amount || 0}
                  </td>
                  <td style={{ padding: '0.85rem 1rem', color: 'var(--ink-subtle)' }} className="capitalize">{item.type || '—'}</td>
                  <td style={{ padding: '0.85rem 1rem' }}>
                    <span style={{ 
                      background: item.status === 'completed' ? 'var(--success-quiet)' : 'var(--warning-quiet)', 
