...
export default function MobilityTab({ API_BASE, authHeaders }) {
  ...
  useEffect(() => {
    setData([
      { id: 'm1', module: 'Bike-Taxi Active Agents', count: 320, status: 'Active' },
      { id: 'm2', module: 'Rural Auto Network', count: 145, status: 'Active' },
      { id: 'm3', module: 'Carpool Routes', count: 86, status: 'Active' },
      { id: 'm4', module: 'Vehicle Rentals', count: 22, status: 'Audit Pending' },
    ]);
    setLoading(false);
  }, []);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      <div style={cardStyle}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
          <div>
            <h3 style={{ fontSize: '1.1rem', margin: 0, color: 'var(--ink)' }}>🚗 Mobility & Transport Control</h3>
            <p style={{ color: 'var(--ink-subtle)', fontSize: '0.85rem', margin: '0.2rem 0 0' }}>Manage local ridesharing, auto networks, and rentals.</p>
          </div>
          <button style={btnPrimary}>View Live Map</button>
        </div>
        
        {loading ? <p style={{ color: 'var(--ink-muted)' }}>Loading mobility metrics...</p> : (
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.88rem' }}>
              <thead>
                <tr>
                  <th style={thStyle}>Sub-Module</th>
                  <th style={thStyle}>Active Agents / Routes</th>
