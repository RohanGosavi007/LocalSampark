...
export default function SocietyTab({ API_BASE, authHeaders }) {
  ...
  const fetchData = async () => {
    ...
    setError(null);
    try {
      const data = await fetchJson(`${API_BASE}/admin/societies?limit=50`, { headers: authHeaders() });
      ...
      <div style={cardStyle}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
          <div>
            ...
            <p style={{ color: 'var(--ink-subtle)', fontSize: '0.85rem', margin: 0 }}>Manage onboarded residential societies, verify committee members, and monitor society-specific activities.</p>
          </div>
            ...
            <tbody>
              {dataList.length === 0 ? (
                ...
                <tr key={item.id || i} style={{ borderBottom: '1px solid var(--line)' }}>
                  ...
                  <td style={{ padding: '0.85rem 1rem', color: 'var(--ink-muted)' }}>{item.pincode || '—'}</td>
                  <td style={{ padding: '0.85rem 1rem', color: 'var(--ink-subtle)' }}>{item.total_flats || 0}</td>
                  <td style={{ padding: '0.85rem 1rem', color: 'var(--ink-muted)' }}>—</td>
                  <td style={{ padding: '0.85rem 1rem' }}>
                    <span style={{ 
                      background: item.is_active ? 'var(--success-quiet)' : 'var(--accent-quiet)', 
                      color: item.is_active ? 'var(--success)' : 'var(--accent-text)', 
