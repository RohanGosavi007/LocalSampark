...
export default function CRMTab({ API_BASE, authHeaders }) {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE}/admin/crm/leads`, { headers: authHeaders() });
      ...
      <div style={cardStyle}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
          <div>
            ...
            <p style={{ color: '#64748b', fontSize: '0.85rem', margin: 0 }}>Track user engagement metrics, manage push notification campaigns, and analyze retention.</p>
          </div>
            ...
            <tbody>
              {dataList.length === 0 ? (
                ...
                <tr key={item.id || i} style={{ borderBottom: '1px solid #1e293b' }}>
                  ...
                  <td style={{ padding: '0.85rem 1rem', color: '#94a3b8' }}>{item.name || '—'}</td>
                  <td style={{ padding: '0.85rem 1rem', color: '#94a3b8' }}>{item.phone || '—'}</td>
                  <td style={{ padding: '0.85rem 1rem', color: '#64748b' }}>{item.source || '—'}</td>
                  <td style={{ padding: '0.85rem 1rem' }}>
                    <span style={{ 
                      background: item.status === 'converted' ? '#052e16' : '#1e1b4b', 
                      color: item.status === 'converted' ? '#4ade80' : '#a5b4fc', 
